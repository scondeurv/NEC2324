...
public sealed class MyNeuralNetwork
{
    ...
    private (double Mse, double Mape)[] TrainingErrors { get; set; }
        ...
        double learningRate, double momentum)
    {
        ...
        Facts = facts.Select(f => factory.Create(f)).ToArray();
    }
    ...
    public async Task Fit(double[][] data, string[] features, CancellationToken? cancellationToken = null)
    {
        ...
        for (var epoch = 0; epoch < Epochs; epoch++)
        {
            ...
            ValidationErrors[epoch] =
                await CalculateErrors(datasets.ValidationSet, cancellationToken);
        }
    ...
    private void Init()
    {
        ...
        TrainingErrors = new (double Mse, double Mape)[Epochs];
        ValidationErrors = new (double Mse, double Mape)[Epochs];
    }
    ...
    private Task BackPropagation(double y, double z, CancellationToken? cancellationToken)
    {
        for (var i = 0; i < N[L - 1]; i++)
        {
            ...
            Delta[L - 1][i] = Facts[L - 1].Derivative(H[L - 1][i]) * (y - z);
        }
        ...
        for (var layer = L - 1; layer > 1; layer--)
        {
            ...
            for (var j = 0; j < N[layer - 1]; j++)
            {
                ...
                var gPrime = Facts[layer - 1].Derivative(H[layer - 1][j]);
    ...
    private async Task<(double Mse, double Mape)> CalculateErrors(double[][] data, CancellationToken? cancellationToken)
    {
        ...
        var mape = 100 * sumMape / data.Length;
        return (mse, mape);
    }
}